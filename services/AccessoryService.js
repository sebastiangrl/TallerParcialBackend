...
(function (AccessoryService) {
    ...
    function eliminar(id) {
        const query = Model_1.default.delete("accessorys", id);
        return query;
    }
    ...
    function update(id, name, price, damage, precision, scope, cadence, mobility, control, level, type) {
        const query = Model_1.default.update("accessorys", `SET name = '${name}', price = ${price}, damage = ${damage}, prec = ${precision}, scope = ${scope}, cadence = ${cadence}, mobility = ${mobility}, control = ${control}, level = ${level}, type_id = ${type} `, id);
        return query;
    }
    ...
    function getByType(type) {
        const query = Model_1.default.select("accessorys", `WHERE type_id LIKE '%${type}%'`);
        return query;
    }
